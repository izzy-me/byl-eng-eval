...
I used LLMs throughout the project. Firstly, I read the assignment spec several times and then had the LLM (chatgpt) read it over, and I asked guiding questions to make sure I understood what was being asked. I also used an LLM to help me understand the already given API endpoint design (for roles) so that I could build a new API endpoint that returned user results. Catastrophe hit and my computer ran out of storage. I used Chatgpt to help me debug whatever was happening with VSCode (VSCode couldn't save anything because of my lack of storage and so it was in an indefinite state of loading), which resulted in the loss of all my files for this project. I had to redo the getRoleAction.ts file and the API endpoint file route.ts under app/api/userResults because I hadn't yet pushed them to GitHub. Lesson hopefully learned!
